...
import type { AnalyzedMove, GameAnalysis, MoveClassification } from "@/modules/game-review/analyzer";
import { Chess } from "chess.js";

// Dynamic import for Chessboard (SSR incompatible)
const Chessboard = dynamic(() => import("react-chessboard").then((m) => m.Chessboard), {
  ssr: false,
  loading: () => <div className="w-full aspect-square bg-slate-800/40 rounded-lg animate-pulse" />,
});

const CLASSIFICATION_LABELS: Record<MoveClassification, { label: string; color: string }> = {
...
function AnalysisSummary({ analysis, playerColor }: { analysis: GameAnalysis; playerColor: "white" | "black" }) {
  ...
  const inaccuracies = isWhite ? analysis.inaccuracies.white : analysis.inaccuracies.black;

  return (
    <div className="grid grid-cols-2 gap-3">
      <div className="bg-slate-800/40 rounded-lg p-3 col-span-2">
        <div className="text-xs text-slate-500 uppercase tracking-wide mb-1">Your Accuracy</div>
...
export default function GameReviewPage() {
  ...
  // Fetch game PGN and info
  useEffect(() => {
    async function fetchGame() {
      try {
        // Fetch from the games API to find this specific game
        ...
        setGameInfo({
          ...
          whiteElo: game.playerColor === "white" ? String(game.playerRating) : String(game.opponentRating),
          blackElo: game.playerColor === "black" ? String(game.playerRating) : String(game.opponentRating),
          result: game.result === "win" ? (game.playerColor === "white" ? "1-0" : "0-1") : game.result === "loss" ? (game.playerColor === "white" ? "0-1" : "1-0") : "½-½",
          date: typeof game.date === "string" ? new Date(game.date).toLocaleDateString() : game.date.toLocaleDateString(),
          opening: game.opening,
          playerColor: game.playerColor,
        });

        // Run Stockfish analysis
        const analysisRes = await fetch("/api/game-review", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ pgn: game.pgn, depth: 18 }),
        });
  ...
  const getCurrentFen = useCallback(() => {
    ...
    return analysis.moves[currentMoveIndex]?.fen ?? "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1";
  ...
  if (currentMove) {
    ...
    // Show best move if it was different
    if (
      currentMove.bestMove &&
      currentMove.bestMove !== currentMove.move &&
      ["blunder", "mistake", "inaccuracy"].includes(currentMove.classification)
    ) {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          {/* Left: Board + Eval */}
          <div className="flex gap-2 lg:w-[520px] shrink-0">
            {/* Eval bar */}
            <div className="relative h-[480px] w-8">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4">
              ...
              <div className="bg-slate-800/30 border border-slate-700/30 rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className="text-lg font-bold font-mono">{currentMove.san}</span>
                    <span className={`text-sm font-semibold ${CLASSIFICATION_LABELS[currentMove.classification].color}`}>
                  ...
                  currentMove.bestMoveSan && (
                    <div className="text-sm text-slate-400">
                      ...
                      <span className="text-slate-600 ml-2">
                        ({currentMove.evalDrop > 0 ? "+" : ""}{(currentMove.evalDrop / 100).toFixed(1)} pawns)
                      </span>
                    </div>
                  )}
              </div>
            ...
            {/* Summary */}
            {analysis && gameInfo && (
              <AnalysisSummary analysis={analysis} playerColor={gameInfo.playerColor} />
            )}
            ...
            {analysis && (
              <div className="bg-slate-800/30 border border-slate-700/30 rounded-xl p-4 flex-1">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-slate-400">Moves</h3>
                  <div className="flex gap-1">
                    ...
                    <button
                      onClick={() => setCurrentMoveIndex((prev) => Math.min(analysis.moves.length - 1, prev + 1))}
                      className="px-2 py-1 text-xs bg-slate-700/50 text-slate-400 rounded hover:bg-slate-700 transition-colors"
                    >
                      ⟩
                    </button>
